...
describe('createSessionTab', () => {
  it('creates a session tab with terminal viewMode', () => {
    const tab = createSessionTab({ label: 'dev-server', hostId: 'mlab', sessionName: 'dev-server', sessionCode: 'abc123' })
    ...
    expect(tab.data.sessionCode).toBe('abc123')
    expect(tab.icon).toBe('TerminalWindow')
  })

  it('creates a session tab with stream viewMode', () => {
    const tab = createSessionTab({ label: 'claude', hostId: 'mlab', sessionName: 'claude', sessionCode: 'def456', viewMode: 'stream' })
    expect(tab.type).toBe('session')
    expect(tab.viewMode).toBe('stream')
    expect(tab.data.sessionName).toBe('claude')
    expect(tab.data.sessionCode).toBe('def456')
  })
})
...
describe('isStandaloneTab', () => {
  it('returns true when tab is not in any workspace', () => {
    const tab = createSessionTab({ label: 'misc', hostId: 'mlab', sessionName: 'misc', sessionCode: 'msc001' })
...
describe('pinned / locked defaults', () => {
  it('createSessionTab defaults pinned=false, locked=false', () => {
    const tab = createSessionTab({ label: 'x', hostId: 'local', sessionName: 'x', sessionCode: 'xxx001' })
